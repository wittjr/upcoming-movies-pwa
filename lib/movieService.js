...
class ServiceMovies {
    ...
    async get_movie_watch_list() {
        ...
        Logger.log(`MovieService.get_movie_watch_list - /api/lists response - ${response.statusText}`)
        if (!response.ok) {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            newData.watchlist = 1
            let oldData = await DatabaseService.getMovie(newData.id)
            Logger.log(`Updating ${newData.id}`)
            // if (currentWatchList.includes(newData.id)) {
            //     Logger.log('Ok')
            //     currentWatchList.rem
            // }
